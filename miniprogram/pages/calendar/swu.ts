...
Page({ 
  ...
  let date1 = e.detail.value.name;
  let dateArr11 = this.data.agenda;
  var flag;
  for(let i = 0; i < this.data.eventsArray.length; i++) {
    var str = this.data.eventsArray[i][1];
    var result =str.includes(date1);
      if(result){
        flag = result;
        let datestr=this.data.eventsArray[i][0];
        dateArr11 = dateArr11+str+","+datestr+"\n";
      }
    }
  if(date1&&flag){
    wx.navigateTo({
      url: '/pages/testadd?data=' + dateArr11
    })
  }
  else{
    wx.showModal({
